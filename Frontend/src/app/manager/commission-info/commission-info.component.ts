...
export class CommissionInfoComponent implements OnInit {
  ...
  public getCommission(): void {
    this.commissionService.getCommission(this.route.snapshot.params.id).subscribe(commisions => {
  ...
  public approveCommission(): void {
    ...
    commissionUpdate.isApprovedByManager = true;
    this.commissionService.updateCommission(this.route.snapshot.params.id, commissionUpdate).subscribe(commission => {
  ...
  public dissapproveCommission(): void {
    const commissionUpdate = this.commission;
    commissionUpdate.isApprovedByManager = false;
    this.commissionService.updateCommission(this.route.snapshot.params.id, commissionUpdate).subscribe(commission => {
      this.commission = commission;
    })
  }
